...
The `XORChipher` will be parameterized to have `numWords` words of storage of width `width`. When it is first turned on, the `XORCipher` will zero out all of its memory. When the user gives the `load` command, they will send the input data and key once per cycle for `numWords` cycles. To read the data decrypted, they will give the `read` command with the key. It will output the data decrypted over the next `numWords` cycles. The user can also give the `clear` command to zero out the memory.
